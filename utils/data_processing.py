...
def get_simulate_data(dataset_name):
    from tgnnexplainer import ROOT_DIR
    data_dir = ROOT_DIR/'xgraph'/'models'/'ext'/'tgat'/'processed'
    graph_df_file = data_dir/'ml_{}.csv'.format(dataset_name)
    edge_features_file = data_dir/'ml_{}.npy'.format(dataset_name)
    node_features_file = data_dir/'ml_{}_node.npy'.format(dataset_name)

    graph_df = pd.read_csv(graph_df_file)
    edge_features = np.load(edge_features_file)
    ...
    used_mask = (labels == 1) | (labels == 0)
    train_flag = np.zeros((len(used_mask),)).astype(bool)
    train_num = int(len(train_flag)*0.8)
    train_flag[:train_num] = True
    test_flag = (1 - train_flag).astype(bool)

    train_mask = used_mask & train_flag
    test_mask = used_mask & test_flag
...
def get_data(dataset_name, different_new_nodes_between_val_and_test=False, randomize_features=False):
    ...
    # Assuming this file is somewhere inside the repo
    ROOT_DIR = Path(__file__).resolve().parents[4]  # adjust number as needed
    print(ROOT_DIR)
