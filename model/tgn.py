...
from modules.message_function import (
    get_message_function,
)
from modules.memory_updater import (
    get_memory_updater,
)
from modules.embedding_module import (
    ...
    def __init__(
        ...
        if self.use_memory:
            ...
            self.message_aggregator = get_message_aggregator(
                aggregator_type=aggregator_type, device=self.device
            )
    ...
    def compute_temporal_embeddings(
        ...
        if self.use_memory and (not self.forbidden_memory_update):
            ...
            if not self.memory_update_at_start:
                unique_node_ids = np.unique(
                    np.concatenate((unique_sources, unique_destinations))
                )
                self.update_memory(unique_node_ids, self.memory.messages)
